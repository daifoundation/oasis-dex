...
context('no escrow, erc20 MKR/DAI market', () => {
  ...
  it('testBuyPosOk', async () => {
    ...
    // mid price
    const { position: thirdBuyPosition } = await alice.buy(mkr(1), dai(550), secondBuyPosition)
    const { prev: prev3, next: next3 } = await orderBook.buyOrder(thirdBuyPosition)
    expect(prev3, 'prev3 should equal secondBuyPosition').to.eq(secondBuyPosition)
    expect(next3, 'next3 should equal firstBuyPosition').to.eq(firstBuyPosition)
    expect(await orderBook.isSorted()).to.be.true
    ...
    expect(prev4, 'prev4 should equal 0').to.eq(0)
